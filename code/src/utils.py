...
def attr_flag(s):
    ...
    attr = s.split(',')
    assert all(attr)
    assert len(attr) == len(set(attr)) >= 1
    assert attr == sorted(attr)
    return attr


def json_list(s):
    """
    Parse stringyfied JSON list.
    """
    if s == '':
        return []
    v = json.loads(s)
...
def json_dict(s):
    """
    Parse stringyfied JSON dictionary.
    """
    if s == '':
        return {}
    v = json.loads(s)
...
def initialize_exp(params, logger_filename='train.log'):
    ...
    if getattr(params, 'train_bt', False):
...
def update_lambdas(params, n_iter):
    ...
    """
    for name in ['lambda_ae', 'lambda_bt', 'lambda_dis', 'lambda_lm', 'otf_temperature']:
...
def get_mask(lengths, all_words, expand=None, ignore_first=False, batch_first=False, cuda=True):
    ...
    """
    bs, slen = lengths.size(0), lengths.max().item()
